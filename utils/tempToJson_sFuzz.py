...
def merge_chains(data, merge_method):
    ...
    for i in range(len(merged_chain)):
        ...
        if merge_method == "max":
            ...
            merged_chain[i]["coverage"] = min(coverage_values)
            merged_chain[i]["num_vulnerability"] = min(num_vulnerability_values)
        elif merge_method == "avg":  # average
            merged_chain[i]["coverage"] = sum(coverage_values) / num_chains
            merged_chain[i]["num_vulnerability"] = sum(num_vulnerability_values) / num_chains
        else:
            raise ValueError(f"Invalid merge method: {merge_method}!!\n Available merge methods: 'max', 'min', 'avg'")
...
def tempToJson():
    ...
    parser.add_argument("-a", "--merge_method", type=str, choices=["max", "min", "avg"], default="max", help="Method to merge chains (default: max)")
    ...
    temp_file_pattern = 'temp_' + args.sol_name + '_phase1_result.txt'
