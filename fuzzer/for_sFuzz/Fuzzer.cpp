...
//  auto it = find_if(first, last, predicate);
//  /* add order sequence */
//  auto orderS = fuzzParam.orderSeq;
//  it->orderSequence = orderS;
//  return *it;
//}
...
void Fuzzer::showStats(const Mutation &mutation, const tuple<unordered_set<uint64_t>, unordered_set<uint64_t>> &validJumpis) {
  ...
  printf(bH " total execs : %s" bH " Br coverage : %s" bH "\n", allExecs.c_str(), coverage.c_str());
...
//      // 获取跟踪位、谓词和异常信息
////      printTracebits();
//      cout << "Tracebits (" << item.res.tracebits.size() << "):" << endl;
//      for (const auto& tb : item.res.tracebits) {
//          cout << "  " << tb << endl;
//      }
//
////      printPredicates();
//      cout << "Predicates (" << item.res.predicates.size() << "):" << endl;
//      for (const auto& pred : item.res.predicates) {
//          cout << "  " << pred.first << ": " << pred.second.str() << endl;
//      }
////      printExceptions();
//      cout << "Exceptions (" << item.res.uniqExceptions.size() << "):" << endl;
//      for (const auto& exc : item.res.uniqExceptions) {
//          cout << "  " << exc << endl;
//      }
//      flag = 1;  // 设为 0 的话，就继续 fuzzing；也可以在这个if开头，加一个if(flag)就跳过，相当于这个代码块只执行一次
//  }
  return item;
}

/* Stop fuzzing */
void Fuzzer::stop(int exitState) {  // new add the state when exit
  Logger::debug("== TEST ==");
...
void Fuzzer::start() {
  ...
  int gapTime = fuzzParam.gapTime;
  int vfcdID = fuzzParam.vfcsID;
...
//    ContractABI ca(contractInfo.abiJson);
    ContractABI ca(contractInfo.abiJson, fuzzParam.orderSeq, contractInfo.isMain);
...
//      ifstream jsonFile("/root/fuzzing/field_test/jsonVFCS/GuessNum.json");
//      stringstream jsonStream;
//      jsonStream << jsonFile.rdbuf();
//      string jsonData = jsonStream.str();
//
//      // 执行确切的函数调用序列
//      TargetContainerResult execResult = executive.execSpecial(jsonData, validJumpis);

      // 打印报告
//      printReport(execResult);
      ...
      if (coverageCalculate(validJumpis) == 1) {
        ...
        string fileName = "temp_" + contractName + "_phase1_result_" + to_string(vfcdID) + ".txt";
...
//          cout << "*********\n"
//                  "fuzzStat.totalExecs is: " << fuzzStat.totalExecs << ". At that round,"
//                  " the data as input from the function `saveIfInterest` is:\n" << data << endl;
...
//            cout << "contractName is: " << contractName << endl;
//            cout << "contract.contractName is: " << contract.contractName << endl;
//            cout << "contractInfo.contractName is: " << contractInfo.contractName << endl;
...
//        double lastRecordTime = 0;
        ...
        if (!(static_cast<int>(timer.elapsed()) % gapTime) && (timer.elapsed() - lastRecordTime) >= gapTime) {
...
//        if (currentTime - lastRecordTime >= gapTime) {
//            ofstream outFile(fileName, ios::app); // Open the file in append mode
//            if (outFile.is_open()) {
//                outFile << "ID: " << ++id << endl;
//                outFile << "Elapsed time: " << static_cast<int>(currentTime) << " seconds" << endl;
//                outFile << "Coverage: " << coverageCalculate(validJumpis) << endl;
//                outFile << "Vulnerability: " << vulnerabilityCalculate() << endl;
//                outFile << "------------------------" << endl;
//                outFile.close();
