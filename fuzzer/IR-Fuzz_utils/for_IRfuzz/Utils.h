...
string fuzzJsonFiles(string contracts, string assets, int duration, int case_num, int mode,
    int reporter, string attackerName, bool is_prefuzz, int is_first)
{
    stringstream ret;
    unordered_set<string> contractNames;
    /* search for sol file */
    forEachFile(contracts, ".sol", [&](directory_entry file) {
        ...
        ret << " --reporter " + to_string(reporter);
        ret << " --attacker " + attackerName;
        ret << " --is_first " + to_string(is_first); /* vLFuzz */
        if (is_prefuzz)
        {
            ret << " --prefuzz";
            ret << endl;
...
string fuzzMainJsonFiles(string contracts, string assets, int duration, int case_num, int mode,
    int reporter, string attackerName, bool is_prefuzz, int is_first)
{
    stringstream ret;
    unordered_set<string> contractNames;

    // 1. 生成 fuzzing 命令，只针对主合约文件
    forEachFile(contracts, ".sol", [&](directory_entry file) {
        auto filePath = file.path().string();
        auto contractName = toContractName(file);

        // 获取文件所在的文件夹名称，假设这是主合约名称
        auto parentFolder = file.path().parent_path().filename().string();

        // 判断文件名是否与父文件夹名称一致，若一致则认为是主合约
        if (contractName != parentFolder) {
            // 如果文件名不等于父文件夹名称，则跳过
            return;
        }
