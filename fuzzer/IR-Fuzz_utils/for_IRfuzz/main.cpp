...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()(
        ...
        /* vLFuzz */
        "is_first", po::value(&isFirst), "vLFuzz the first loop")(
    ...
    if (vm.count("generate"))
    {
        ...
        // fuzzMe << fuzzJsonFiles(contractsFolder, assetsFolder, duration, case_num, mode, reporter,
        //     attackerName, vm.count("prefuzz"), isFirst); /* add `isFirst` */
        fuzzMe << fuzzMainJsonFiles(contractsFolder, assetsFolder, duration, case_num, mode, reporter,
            attackerName, vm.count("prefuzz"), isFirst); /* add `isFirst` */
        fuzzMe.close();
        showGenerate();
        return 0;
