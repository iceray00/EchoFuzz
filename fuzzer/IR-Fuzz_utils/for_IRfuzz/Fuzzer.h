...
public:
    ...
    /* vLFuzz */
    double coverageCalculate(const tuple<unordered_set<uint64_t>, unordered_set<uint64_t>> &validJumpis);
    int vulnerabilityCalculate();
    void stop(int exitState);
    void saveTracebits(const unordered_set<string>& tracebits, const string& filename);
    unordered_set<string> loadTracebits(const string& filename);
};
}  // namespace fuzzer
