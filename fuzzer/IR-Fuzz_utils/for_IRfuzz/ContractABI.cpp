...
ContractABI::ContractABI(string abiJson)
{
    ...
    for (auto& node : root)
    {
        if (node.second.find("order") != node.second.not_found())
        {
            auto orderNode = node.second.get_child("order");
            if (orderNode.empty())
            {
                int order = orderNode.get_value<int>();
                maxOrder = std::max(maxOrder, order);
            }
            else
            {
                for (auto& orderValue : orderNode)
                {
                    int order = orderValue.second.get_value<int>();
                    maxOrder = std::max(maxOrder, order);
                }
            }
        }
    ...
    for (auto& node : root)
    {
        ...
        string constant = "false";
        bool payable = false;
        if (node.second.find("constant") != node.second.not_found())
        {
            constant = node.second.get<string>("constant");
        }
        ...
        if (type == "fallback")
        {
            if (node.second.find("payable") != node.second.not_found())
            {
                payable = node.second.get<bool>("payable");
            }
    ...
    // Store the function name in the order of the order field to functionCallSequence
    for (const auto& fd : this->fds)
    {
        if (!fd.name.empty())
        {
            this->functionCallSequence.push_back(fd.name);
        }
    }
}
