...
void Fuzzer::showStats(const Mutation& mutation, const int branchSize)
{
    ...
    if (fuzzParam.is_prefuzz)
        printf(cGRN Bold "%sEchoFuzz branch seaching (%s)" cRST "\n", padStr("", 10).c_str(),
               contract.contractName.substr(10, contract.contractName.find(":") - 14).c_str());
    else
        printf(cGRN Bold "%sEchoFuzz vulernability detecting (%s)" cRST "\n", padStr("", 10).c_str(),
               contract.contractName.substr(10, contract.contractName.find(":") - 14).c_str());
...
double Fuzzer::coverageCalculate(const tuple<unordered_set<uint64_t>, unordered_set<uint64_t>> &validJumpis) {
    auto totalBranches = (get<0>(validJumpis).size() + get<1>(validJumpis).size()) * 2;
    return static_cast<double>(tracebits.size()) / totalBranches;
}

int Fuzzer::vulnerabilityCalculate() {
    int numCount = 0;
    for (auto Vul : vulnerabilities){
        if (Vul) numCount++;
    }
    return numCount;
}
...
void Fuzzer::start()
{
    ...
    for (auto contractInfo : fuzzParam.contractInfo)
    {
        ...
        else if (fuzzParam.is_prefuzz)  //                                     prefuzz main
        {
            ...
            auto totalBranches = (get<0>(validJumpis).size() + get<1>(validJumpis).size()) * 2;
            // Extract the total branch content and convert it into code snippets
            vector<string> totalBranchSnippets;
            for (const auto &jumpi : get<0>(validJumpis)) {
                totalBranchSnippets.push_back("##(true branch)##\n" + snippets[jumpi]);
                totalBranchSnippets.push_back("##(false branch)##\n" + snippets[jumpi]);
            }
            for (const auto &jumpi : get<1>(validJumpis)) {
                totalBranchSnippets.push_back("##(true branch)##\n" + snippets[jumpi]);
                totalBranchSnippets.push_back("##(false branch)##\n" + snippets[jumpi]);
            }

            /* test */
            // cout << "Total Branches: (" << totalBranchSnippets.size() << ")" << endl;
            // int tmp_show = 0;
            // for (const auto &snippet : totalBranchSnippets) {
            //     cout << "*** The " << ++tmp_show << "th valid Branch: ***" << endl;
            //     cout << snippet << "\n----------------\n" << endl;
            // }

            auto contract = mainContract();
            string fullContractName = contract.contractName;
            size_t colonPos = fullContractName.find(':');
            string contractNameOnly = (colonPos != std::string::npos) ? fullContractName.substr(colonPos + 1) : fullContractName;

            string tbsFileName = "totalBranchSnippet_" + contractNameOnly + ".txt";
            ofstream totalBranchSnippet(tbsFileName);
            if (!totalBranchSnippet.is_open()) {
                cerr << "Error: Unable to open file " << tbsFileName << endl;
            } else {
                totalBranchSnippet << "All Branches: (total " << totalBranchSnippets.size() << ")" << endl;
                int tbs_show = 0;
                for (const auto &snippet : totalBranchSnippets) {
                    totalBranchSnippet << "*** The " << ++tbs_show << "th valid Branch: ***" << endl;
                    totalBranchSnippet << snippet << "\n----------------\n" << endl;
                }
                totalBranchSnippet.close();
                std::cout << "save [" << tbsFileName << "] Successfully!" << endl;
            }

            /* save current function call sequence(VFCS) */
            string sequenceFileName = "sequence_" + contractNameOnly + ".txt";
            ofstream sequence(sequenceFileName);
            if (sequence.is_open()) {
                int seqInd = 0;
                for (auto &funcName : ca.functionCallSequence) {
                    if (!seqInd) {
                        seqInd = 1;
                        sequence << funcName;
                    }
                ...
                sequence.close();
                cout << "save [" << sequenceFileName << "] Successfully!" << endl;
            } else {
                cerr << "Error: Unable to open file " << sequenceFileName << endl;
            }

            /*############## load `tracebits` from previous fuzzing ###############*/
            if (!isFirst) {
                cout << "Enter the `!isFirst` !!" << endl;
                string tracebitsFileName = "tracebits_" + contractNameOnly +".txt";
            ...
            auto extractTracebits = [&]() {
                ...
                for (const auto &tracebit : tracebits) {
                    size_t colonPos = tracebit.find(':');
                    if (colonPos != string::npos) {
                        uint64_t pc = stoull(tracebit.substr(0, colonPos));
                        if (snippets.find(pc) != snippets.end()) {
                            tracebitSnippets.push_back(snippets[pc]);
                        }
                    }
                }
            ...
            /* testing */
            /* show the tracebits in source code snippet */
            // vector<string> tracebitSnippets = extractTracebits();
            // cout << "Covered Branches (" << tracebitSnippets.size() << "):" << endl;
            // int tmp_cb = 0;
            ...
            */
            ...
            int flagFirst = 0; /* the First time */

            /* coverage == 100% */
            if (coverageCalculate(validJumpis) == 1) {
                auto contract = mainContract();
                string fullContractName = contract.contractName;
                size_t colonPos = fullContractName.find(':');
                string contractName = (colonPos != std::string::npos) ? fullContractName.substr(colonPos + 1) : fullContractName;
                string fileName = "temp_" + contractName + "_phase1_result.txt";

                ofstream outFile(fileName, ios::app);
                if (outFile.is_open()) {
            //          outFile << "ID: " << ++id << endl;
            //          outFile << "Elapsed time: " << timer.elapsed() << " seconds" << endl;
            //          outFile << "Coverage: " << coverageCalculate(validJumpis) << endl;
            //          outFile << "Vulnerability: " << vulnerabilityCalculate() << endl;
                    outFile << ++id << endl;
                    outFile << static_cast<int>(timer.elapsed()) << endl;
                    outFile << 1 << endl;
            ...
            while (true)
            {
                ...
                auto save = [&](bytes data) {  //           using save to control execute on
                        ...
                        speed <= 10 || !predicates.size())
                    {
                        ...
                        /* save coverage/num_vulnerability information file before STOP fuzzing */
                        ofstream outFile(fileName, ios::app);
                        if (outFile.is_open()) {
            //                outFile << "ID: " << ++id << endl;
            //                outFile << "Elapsed time: " << timer.elapsed() << " seconds" << endl;
            ...
            //                cout << "Result save in [" << fileName << "] successfully!" << endl;
                ...
                string contractName = (colonPos != std::string::npos) ? fullContractName.substr(colonPos + 1) : fullContractName;
                string fileName = "temp_" + contractName + "_phase1_result.txt";
                /* save in the First Fuzzing Loop at the first time*/
                if (!flagFirst && isFirst) {
                    flagFirst = 1;
                    ofstream outFile(fileName);
        //            if (outFile.is_open()) {
        //                outFile << "ID: 0" << endl;
        //                outFile << "Elapsed time: 0" << endl;
        //                outFile << "Coverage: " << coverageCalculate(validJumpis) << endl;
        //                outFile << "Vulnerability: " << vulnerabilityCalculate() << endl;
        //                outFile << "------------------------" << endl;
        //                outFile.close();
        //            }
                ...
                /* for each gap time, record the stat of fuzzing! */
        //        double lastRecordTime = 0;
                // If defined here, the fuzzing would be performed and reset every time throughout the fuzzing process! So it needs to be outside the while loop of fuzzing
                // if (!(static_cast<int>(timer.elapsed()) % gapTime) && (timer.elapsed() - lastRecordTime) >= gapTime) {  /* gapTime == 1 */
                if (!(static_cast<int>(timer.elapsed()) % 1) && (timer.elapsed() - lastRecordTime) >= 1) {
                    ofstream outFile(fileName, ios::app); // Open the file in append mode
                    if (outFile.is_open()) {
        //                outFile << "ID: " << ++id << endl;
        //                outFile << "Elapsed time: " << static_cast<int>(timer.elapsed()) << " seconds" << endl;
        //                outFile << "Coverage: " << coverageCalculate(validJumpis) << endl;
        //                outFile << "Vulnerability: " << vulnerabilityCalculate() << endl;
        //                outFile << "------------------------" << endl;
                        outFile << ++id << endl;
...
//                        cout << "Result appended to [" << fileName << "] successfully!" << endl;
        ...
        else  //                                                                 main
        {
            ...
            auto validJumpis = bytecodeBranch.findValidJumpis();
            // cout << "iiiiiiiceRRRRAY: coverage is: " << coverageCalculate(validJumpis) << "=========\n" << endl;
            ...
            //     size_t colonPos = fullContractName.find(':');
            //     string contractName = (colonPos != std::string::npos) ? fullContractName.substr(colonPos + 1) : fullContractName;
            //     string fileName = "temp_" + contractName + "_phase1_result_Vulnerability.txt";
    ...
    //             string contractName = (colonPos != std::string::npos) ? fullContractName.substr(colonPos + 1) : fullContractName;
    //             string fileName = "temp_" + contractName + "_phase1_result_Vulnerability.txt";
    //             /* save in the First Fuzzing Loop */
    //             if (!flagFirst && isFirst) {
    //                 flagFirst = 1;
    //                 ofstream outFile(fileName);
    // //            if (outFile.is_open()) {
...
void Fuzzer::saveTracebits(const unordered_set<string>& tracebits, const string& filename) {
    ...
    for (const auto& trace : tracebits) {
        outfile << trace << "\n";
    }
    outfile.close();
